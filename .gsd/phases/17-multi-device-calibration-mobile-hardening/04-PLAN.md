...
must_haves:
  truths:
    - "iOS Safari window scroll is automatically clamped to (0, 0) during virtual keyboard activation, preventing the header and race strip from being pushed off-screen."
    - "Arena layout dynamically scales within compact viewports (<450px visual height), ensuring hero word, race track, and input are never visually cut off."
...
<objective>
Eliminate mobile viewport clipping and scrolling cutoffs by locking window scroll coordinates to (0, 0) on visualViewport changes, implement dynamic scaling for ultra-short mobile viewports, and verify all device calibration, UI updates, and input hardening features with automated tests.
...
<task type="auto">
  <name>Implement iOS Safari Anti-Cutoff Scroll Lock</name>
  <files>src/engine/useDeviceProfile.ts</files>
  <action>
    In `src/engine/useDeviceProfile.ts`:
    - In `handleVvChange` (listening to `visualViewport` resize and scroll):
      - On iOS Safari, opening the keyboard triggers an automatic page scroll (`window.scrollY > 0`), which pushes the header and top race track off-screen with no way to scroll back under `overflow: hidden`.
  ...
  <done>
    Viewport never scrolls off-screen on keyboard pop-up.
  </done>
</task>
